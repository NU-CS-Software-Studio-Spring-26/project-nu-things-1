...
class ProfileAvatarsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "signed-in user can choose first initial avatar" do
    student = users(:nu_student)
    sign_in_as(student)

    patch profile_avatar_path, params: { user: { profile_avatar: "initial" } }

    assert_redirected_to user_path(student)
    assert_equal "initial", student.reload.profile_avatar
  end
