class ContactMailer < ApplicationMailer
  ...
  def lost_item_report(lost_item, reporter_name, reporter_email, details)
    ...
    mail(
      ...
      subject: "[#{Rails.application.config.lofonu_brand_name}] Reported lost item ##{lost_item.id}: #{lost_item.title}"
    )
  end
  ...
  def found_item_report(found_item, reporter_name, reporter_email, details)
    ...
    mail(
      to: listing_moderation_to,
      reply_to: reporter_email,
      subject: "[#{Rails.application.config.lofonu_brand_name}] Reported found item ##{found_item.id}: #{found_item.title}"
    )
  end
