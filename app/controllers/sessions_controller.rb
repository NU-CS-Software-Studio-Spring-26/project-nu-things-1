class SessionsController < ApplicationController
  ...
  def create
    ...
    if northwestern_login_email?(email)
  ...
  def consume
    ...
    return_to = pop_return_to
    reset_session
    session[:user_id] = login_token.user_id

    redirect_to return_to || root_path, notice: "Signed in successfully."
  ...
  def northwestern_login_email?(email)
    email.present? && email.match?(User::NORTHWESTERN_EMAIL)
  end
end
